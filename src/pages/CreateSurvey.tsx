...
const CreateSurveyForm: React.FC<NavigateProps> = ({ navigateTo }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const createdSurvey = await createSurvey(session.user.id, title, questions);
      setMessage(
  ...
  return (
    <div>
      <h2 className="text-3xl font-bold text-center mb-6 text-gray-800">Create New Survey</h2>
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label htmlFor="survey-title" className="block text-lg font-medium text-gray-700 mb-2">
            Survey Title
          </label>
          <input
            type="text"
        ...
        <div className="space-y-4">
          <h3 className="text-xl font-semibold text-gray-700">Questions (Yes/No)</h3>
          {questions.map((q, index) => (
            <div key={q.id} className="p-4 border border-gray-200 rounded-md bg-gray-50">
              ...
              <input
                ...
                required
              />
              <p className="mt-2 text-sm text-gray-500">Options: Yes / No (fixed)</p>
            </div>
          ))}
        </div>
