...
const PublicSurveyPage: React.FC<PublicSurveyPageProps> = ({ surveyId, navigateTo }) => {
  ...
  const [submissionMessageType, setSubmissionMessageType] = useState<'success' | 'error' | ''>('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmissionMessage(err.message || 'An unexpected error occurred during submission.');
      setSubmissionMessageType('error');
    }
  };

  if (loading) {
    return <div className="text-center py-10 text-gray-600">Loading survey...</div>;
  }
