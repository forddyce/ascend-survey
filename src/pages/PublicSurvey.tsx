...
const PublicSurveyPage: React.FC<PublicSurveyPageProps> = ({
  surveyId,
  navigateTo,
}) => {
    ...
    const fetchSurvey = async () => {
      ...
      try {
        ...
        if (now > expiresAt) {
          ...
          setError("This survey has reached its maximum number of votes.");
          setSurvey(null);
        } else {
          setSurvey(data);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!survey) {
      setSubmissionMessage("Survey not loaded or invalid.");
    ...
    if (
      new Date() > new Date(survey.expires_at) ||
      survey.current_votes >= survey.max_votes
    ) {
      setSubmissionMessage("This survey is no longer accepting submissions.");
      setSubmissionMessageType("error");
      return;
    }
    ...
    try {
      ...
      if (result && result.error) {
        ...
        setSubmitted(true);
        setSubmissionMessage("Your submission has been recorded successfully!");
  ...
  if (loading) {
    return (
      <div className="text-center py-10 text-gray-600">Loading survey...</div>
    );
  }
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {survey.questions.map((q) => (
          <div
            key={q.id}
            className="p-5 border border-gray-200 rounded-md bg-gray-50 shadow-sm"
          >
        ...
        <div className="flex justify-center mt-8">
          <button
            type="submit"
            className="bg-blue-600 text-white px-8 py-4 rounded-full shadow-lg hover:bg-blue-700 transition duration-300 ease-in-out transform hover:scale-105"
          >
            Submit Survey
          </button>
        </div>
